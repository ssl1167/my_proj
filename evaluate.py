...
def run_episode(model, env, circuit, device, deterministic: bool = True, use_physical_prior: bool = True):
    ...
    for sample in tqdm(samples, desc="eval"):
        ...
        if args.beam_width > 1:
            ...
            if not args.skip_greedy:
                reward, info = run_episode(model, env, circuit, device, deterministic=True, use_physical_prior=use_physical_prior)
                candidate_runs.append((reward, info, "greedy"))
            for rollout_idx in range(max(0, args.num_rollouts - 1)):
                cand_reward, cand_info = run_episode(model, env, circuit, device, deterministic=False, use_physical_prior=use_physical_prior)
        ...
        row = {"name": sample.name, "family": sample.family, "num_qubits": sample.num_qubits}
